...
public class AddStudentFormController {
    ...
    public void loadStudentForEdit(StudentsController.StudentRow student) {
        try (Connection conn = DBConnection.getConnection()) {
            // Try join using users.user_id first (preferred schema)
            String q1 = "SELECT s.id AS student_id, s.user_id, u.username, u.password, s.name, s.email, " +
            ...
            if (!hasRow) {
                rs.close(); ps.close();
                String q2 = "SELECT s.id AS student_id, s.user_id, u.username, u.password, s.name, s.email, " +
                        "c.name AS course, d.name AS department, sem.name AS semester " +
                        "FROM students s " +
            ...
            if (hasRow) {
                ...
                // populate basic fields defensively
                String name = rs.getString("name");
                if (name != null) studentName.setText(name); else studentName.clear();

                String email = rs.getString("email");
                if (email != null) studentEmail.setText(email); else studentEmail.clear();

                String uname = rs.getString("username");
                if (uname != null) studentUsername.setText(uname); else studentUsername.clear();

                String pwd = rs.getString("password");
                if (pwd != null) studentPassword.setText(pwd); else studentPassword.clear();
