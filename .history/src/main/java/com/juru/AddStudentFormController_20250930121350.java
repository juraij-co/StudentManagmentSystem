...
public class AddStudentFormController {
    ...
    public void loadStudentForEdit(StudentsController.StudentRow student) {
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(
                     "SELECT s.id AS student_id, u.user_id, u.username, u.password, s.name, s.email, " +
            ...
            if (rs.next()) {
                ...
                studentDepartment.setValue(rs.getString("department"));
                loadCourses();
                studentCourse.setValue(rs.getString("course"));
                loadSemesters();
                studentSemester.setValue(rs.getString("semester"));
            }
            rs.close();
