...
import com.juru.models.AttendanceRecord;
...
public class StudentAttendanceController {
    ...
    private int studentUserId;   // user login id
    private int studentDbId;     // id from students table
    private int courseId, semesterId;
    ...
    public void initialize() {
        studentUserId = Session.getInstance().getUserId();
        loadStudentInfo();
        loadSemesters();
        loadMonthComboBox();
        setupTableColumns();
    ...
    private void loadStudentInfo() {
        String sql = "SELECT s.id, s.name, s.course_id, s.semester_id " +
                     "FROM students s WHERE s.user_id=?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pst = conn.prepareStatement(sql)) {
            pst.setInt(1, studentUserId);
            ResultSet rs = pst.executeQuery();
            if (rs.next()) {
                studentDbId = rs.getInt("id");   // real student id for attendance
                studentNameLabel.setText("Welcome, " + rs.getString("name"));
                studentIdLabel.setText("ID: " + studentDbId);
        ...
        try(Connection conn = DBConnection.getConnection();
            ...
            while(rsSub.next()){
                ...
                try(PreparedStatement pstAtt = conn.prepareStatement(sqlAtt)){
                    pstAtt.setInt(1, studentDbId);   // FIXED: use studentDbId not userId
    ...
    private void loadMonthlyAttendance() {
        ...
        monthlyGrid.getColumnConstraints().clear();
        ...
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pst = conn.prepareStatement(
                     "SELECT day_of_week, period_no, subject_id, (SELECT name FROM subjects WHERE id=t.subject_id) AS name " +
                             "FROM timetables t WHERE t.course_id=? AND t.semester_id=?")) {
            pst.setInt(1, courseId);
            pst.setInt(2, semesterId);
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                String day = rs.getString("day_of_week").toUpperCase(); // FIXED
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            pst.setInt(1, studentDbId);   // FIXED
            pst.setInt(2, month);
            pst.setInt(3, year);
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                String key = rs.getDate("date").toString() + "_" + rs.getInt("period_no") + "_" + rs.getInt("subject_id");
                attendanceMap.put(key, rs.getString("status"));
            }
        } catch (SQLException e) { e.printStackTrace(); }

        // Grid constraints
        for (int c = 0; c <= periods; c++) monthlyGrid.getColumnConstraints().add(new ColumnConstraints(100));
        for (int r = 0; r <= days; r++) monthlyGrid.getRowConstraints().add(new RowConstraints(40));

        // Fill grid
        for (int d = 1; d <= days; d++) {
            LocalDate date = LocalDate.of(year, month, d);
            String dow = date.getDayOfWeek().toString(); // MONDAY
            String dbDay = dow.toUpperCase();           // "MONDAY"

            // Date label
            Label dateLabel = new Label(String.valueOf(d));
            dateLabel.setMaxSize(Double.MAX_VALUE, Double.MAX_VALUE);
            dateLabel.setStyle("-fx-border-color: #dee2e6; -fx-alignment: center; -fx-font-weight: bold;");
            monthlyGrid.add(dateLabel, 0, d);

            for (int p = 1; p <= periods; p++) {
                SubjectInfo si = timetableMap.getOrDefault(dbDay, new HashMap<>()).get(p);

                Rectangle rect = new Rectangle(100, 40);
                rect.setStroke(Color.web("#bdbdbd"));
                Label lbl = new Label(si != null ? si.name : "");
                lbl.setMaxSize(Double.MAX_VALUE, Double.MAX_VALUE);
                lbl.setStyle("-fx-alignment: center; -fx-font-size: 12;");

                if (dow.equals("SATURDAY") || dow.equals("SUNDAY")) rect.setFill(Color.web("#bbdefb"));
                else if (si == null) rect.setFill(Color.web("#ffffff"));
                else {
                    String key = date.toString() + "_" + p + "_" + si.id;
                    String status = attendanceMap.get(key);
                    if ("Present".equalsIgnoreCase(status)) rect.setFill(Color.web("#4caf50"));
                    else if ("Absent".equalsIgnoreCase(status)) rect.setFill(Color.web("#f44336"));
