...
public class StudentAttendanceController {
    ...
    try (Connection conn = DBConnection.getConnection();
        ...
        ResultSet rs = pst.executeQuery();
        while (rs.next()) {
            String day = rs.getString("day_of_week"); // Keep same as DB
    ...
    for (int c = 0; c <= periods; c++) {
        ...
        monthlyGrid.getColumnConstraints().add(cc);
    }
    for (int r = 0; r <= days; r++) {
        RowConstraints rc = new RowConstraints(40);
        monthlyGrid.getRowConstraints().add(rc);
    }

    // Fill grid
    for (int d = 1; d <= days; d++) {
        LocalDate date = LocalDate.of(year, month, d);
        String dow = date.getDayOfWeek().toString(); // MONDAY, TUESDAY, etc.
        String dbDay = capitalizeFirstLetter(dow.toLowerCase()); // "Monday"
