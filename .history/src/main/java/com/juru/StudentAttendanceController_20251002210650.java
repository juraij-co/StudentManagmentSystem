...
public class StudentAttendanceController {
    ...
    try (Connection conn = DBConnection.getConnection();
        ...
        while (rsSub.next()) {
            ...
            String sqlAtt = "SELECT COUNT(*) AS total, SUM(status='Present') AS attended " +
...
private void loadMonthlyAttendance() {
    ...
    // Load student subjects for the semester
    Map<Integer, String> studentSubjects = new HashMap<>();
    try (Connection conn = DBConnection.getConnection();
        ...
        ResultSet rs = pst.executeQuery();
        while (rs.next()) {
            studentSubjects.put(rs.getInt("id"), rs.getString("name"));
        }
    } catch (SQLException e) { e.printStackTrace(); }

    // Load timetable filtered by student subjects
    Map<String, Map<Integer, Integer>> timetableMap = new HashMap<>();
    try (Connection conn = DBConnection.getConnection();
         PreparedStatement pst = conn.prepareStatement(
                 "SELECT day_of_week, period_no, subject_id " +
                 "FROM timetables WHERE semester_id=? AND course_id=?")) {
        pst.setInt(1, semesterId);
        pst.setInt(2, courseId);
        ResultSet rs = pst.executeQuery();
        while (rs.next()) {
            int subjectId = rs.getInt("subject_id");
            if (!studentSubjects.containsKey(subjectId)) continue;
            String day = rs.getString("day_of_week").toUpperCase();
            int period = rs.getInt("period_no");
            timetableMap.computeIfAbsent(day, k -> new HashMap<>()).put(period, subjectId);
        }
    } catch (SQLException e) { e.printStackTrace(); }
    ...
    try (Connection conn = DBConnection.getConnection();
        ...
        while (rs.next()) {
            ...
            int period = rs.getInt("period_no");
            int subjectId = rs.getInt("subject_id");
            String key = date.toString() + "_" + period + "_" + subjectId;
    ...
    for (int day = 1; day <= daysInMonth; day++) {
        ...
        for (int period = 1; period <= periods; period++) {
            Integer subjectId = timetableMap.getOrDefault(dow, new HashMap<>()).get(period);
            String subjectName = subjectId != null ? studentSubjects.get(subjectId) : null;
