...
public class StudentAttendanceController {
    ...
    private void loadSubjectWiseAttendance() {
        ...
        String sqlSubjects = "SELECT sub.id, sub.name FROM subjects sub " +
                             "JOIN student_subject ss ON ss.subject_id=sub.id " +
                             "WHERE ss.student_id=? AND sub.course_id=? AND sub.semester_id=?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pstSub = conn.prepareStatement(sqlSubjects)) {

            pstSub.setInt(1, studentId);
            pstSub.setInt(2, courseId);
            ...
            while (rsSub.next()) {
                ...
                String sqlAtt = "SELECT COUNT(*) AS total, SUM(CASE WHEN status='Present' THEN 1 ELSE 0 END) AS attended " +
    ...
    private void loadMonthlyAttendance() {
        ...
        // Load timetable for the semester
        Map<DayOfWeek, Map<Integer, SubjectInfo>> timetableMap = new HashMap<>();
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pst = conn.prepareStatement(
                     "SELECT day_of_week, period_no, subject_id, (SELECT name FROM subjects WHERE id=t.subject_id) AS subject_name " +
                     "FROM timetables t " +
            ...
            while (rs.next()) {
                ...
                timetableMap.computeIfAbsent(dow, k -> new HashMap<>())
                        .put(period, new SubjectInfo(subjId, subjName));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                attendanceMap.put(date.toString() + "_" + period + "_" + subjId, status);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
        ...
        for (int day = 1; day <= daysInMonth; day++) {
            ...
            for (int period = 1; period <= periods; period++) {
                SubjectInfo subjInfo = timetableMap.getOrDefault(dow, new HashMap<>()).get(period);
                Rectangle rect = new Rectangle(100, 40);
                rect.setStroke(Color.web("#bdbdbd"));
                Label subjLabel = new Label(subjInfo != null ? subjInfo.name : "");
                ...
                else if (subjInfo == null) rect.setFill(Color.web("#ffffff"));
                else {
                    String key = date.toString() + "_" + period + "_" + subjInfo.id;
    ...
    private static class SubjectInfo {
        int id;
        String name;
        SubjectInfo(int id, String name) { this.id = id; this.name = name; }
    }
