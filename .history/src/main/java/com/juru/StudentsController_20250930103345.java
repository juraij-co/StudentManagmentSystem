...
public class StudentsController {
    ...
    private void initialize() {
        ...
        // Add Edit / Delete buttons to the actions column
        colActions.setCellFactory(col -> new TableCell<StudentRow, String>() {
            @Override
            protected void updateItem(String item, boolean empty) {
                ...
                editBtn.setOnAction(evt -> {
                    try {
                        ...
                        formCtrl.setParentController(StudentsController.this);
                        formCtrl.loadStudent(row.getId());
                ...
                delBtn.setOnAction(evt -> {
                    Alert confirm = new Alert(Alert.AlertType.CONFIRMATION, "Delete student '" + row.getName() + "' ?", ButtonType.YES, ButtonType.NO);
                    ...
                    confirm.showAndWait().ifPresent(b -> {
                        if (b == ButtonType.YES) {
                            try (Connection conn = DBConnection.getConnection()) {
                                ...
                                // get user's id
                                int userId = 0;
                                try (PreparedStatement q = conn.prepareStatement("SELECT user_id FROM students WHERE id = ?")) {
                                ...
                                try (PreparedStatement ps1 = conn.prepareStatement("DELETE FROM student_subject WHERE student_id = ?")) {
                                    ps1.setInt(1, row.getId());
                                    ps1.executeUpdate();
                                }

                                try (PreparedStatement ps2 = conn.prepareStatement("DELETE FROM students WHERE id = ?")) {
                                    ps2.setInt(1, row.getId());
                                    ps2.executeUpdate();
                                }

                                if (userId > 0) {
                                    try (PreparedStatement ps3 = conn.prepareStatement("DELETE FROM users WHERE id = ?")) {
                                        ps3.setInt(1, userId);
                                        ps3.executeUpdate();
                                    }
                                }
                                ...
                                conn.commit();
                                statusLabel.setText("Deleted student '" + row.getName() + "'");
                                loadStudentsFromDB();
                            } catch (Exception ex) {
                                ex.printStackTrace();
