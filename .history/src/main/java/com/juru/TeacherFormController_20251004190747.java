...
public class TeacherFormController {
    ...
    /** Loads departments and auto-selects if only one exists */
    private void loadDepartments() {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            teacherDepartment.getItems().setAll(departments);

            // ✅ Auto-select if only one department
            if (departments.size() == 1) {
                teacherDepartment.setValue(departments.get(0));
                loadCourses(departments.get(0)); // Auto-load courses too
            }
