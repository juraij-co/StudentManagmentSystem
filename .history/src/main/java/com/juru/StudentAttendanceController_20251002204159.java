...
public class StudentAttendanceController {
    ...
    private Map<String, Integer> semesterMap = new HashMap<>(); // name -> id
    ...
    private void loadMonthlyAttendance() {
        ...
        Map<Integer, Integer> periodSubjectMap = new HashMap<>();
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pst = conn.prepareStatement(
                     "SELECT day_of_week, period_no, s.name as subject_name, s.id as subject_id " +
            ...
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                String day = rs.getString("day_of_week"); // e.g., "Monday"
                int period = rs.getInt("period_no");
                String subject = rs.getString("subject_name");
                int subjectId = rs.getInt("subject_id");
                timetableMap.computeIfAbsent(day.toUpperCase(), k -> new HashMap<>())
                        .put(period, subject + ":" + subjectId);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
        ...
        for (int day = 1; day <= daysInMonth; day++) {
            ...
            for (int period = 1; period <= periods; period++) {
                String subjData = timetableMap.getOrDefault(dow.name(), new HashMap<>()).get(period);
                String subjectName = null;
                int subjectId = -1;
                if (subjData != null && subjData.contains(":")) {
                    String[] parts = subjData.split(":");
                    subjectName = parts[0];
                ...
                if (dow == DayOfWeek.SATURDAY || dow == DayOfWeek.SUNDAY) {
                    ...
                    String sqlAtt = "SELECT status FROM attendance WHERE student_id=? AND subject_id=? AND date=? AND period_no=?";
                    try (Connection conn = DBConnection.getConnection();
                         PreparedStatement pstAtt = conn.prepareStatement(sqlAtt)) {
                        pstAtt.setInt(1, studentId);
                        pstAtt.setInt(2, subjectId);
                        pstAtt.setDate(3, Date.valueOf(date));
                        pstAtt.setInt(4, period);
                        ResultSet rsAtt = pstAtt.executeQuery();
                        if (rsAtt.next()) {
