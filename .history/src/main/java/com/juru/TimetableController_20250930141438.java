...
/**
 * Minimal controller for Timetable.fxml. Provides basic wiring so the FXML can be loaded
 * and the combo boxes / subject list are populated. This avoids FXMLLoader errors when
...
public class TimetableController {
    ...
    public void initialize() {
        ...
        if (ttDepartmentCombo != null) {
            ...
            ttDepartmentCombo.setConverter(new javafx.util.StringConverter<Department>() {
                @Override
                public String toString(Department dept) { return dept == null ? null : dept.getName(); }
                @Override
        ...
        if (saveTimetableBtn != null) {
            saveTimetableBtn.setOnAction(e -> {
                ...
                if (ttStatusLabel != null) ttStatusLabel.setText("✔ Timetable saved (not persisted in this build)");
            });
        }

        if (ttStatusLabel != null) ttStatusLabel.setText("Ready");
    }
    ...
    private void loadDepartmentsIntoCombo() {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            if (ttStatusLabel != null) ttStatusLabel.setText("❌ Error loading departments: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void loadCoursesForDepartment() {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            if (ttStatusLabel != null) ttStatusLabel.setText("❌ Error loading courses: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void loadSemestersForCourse() {
        ...
        try (Connection conn = DBConnection.getConnection();
             ...
             ResultSet rs = stmt.executeQuery(sql)) {
            while (rs.next()) {
                ttSemesterCombo.getItems().add(new Semester(rs.getInt("id"), rs.getString("name"), c.getName()));
            }
        } catch (Exception e) {
            if (ttStatusLabel != null) ttStatusLabel.setText("❌ Error loading semesters: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void loadSubjectsForSelection() {
        ...
        if (c == null || s == null) {
            if (ttStatusLabel != null) ttStatusLabel.setText("⚠ Select course and semester first");
            return;
        }
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            if (ttStatusLabel != null) ttStatusLabel.setText("Loaded " + subjectsListView.getItems().size() + " subjects");
        } catch (Exception e) {
            if (ttStatusLabel != null) ttStatusLabel.setText("❌ Error loading subjects: " + e.getMessage());
            e.printStackTrace();
        }
    }
