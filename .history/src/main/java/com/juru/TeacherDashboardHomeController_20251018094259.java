...
@FXML
private TableColumn<TimetableEntry, Integer> periodColumn;
@FXML
...
private TableColumn<TimetableEntry, String> courseColumn;
    ...
    try (Connection conn = DBConnection.getConnection()) {
        String sql = """
            SELECT t.day_of_week, t.period_no, s.name AS subject_name,
                   sem.name AS semester_name, c.name AS course_name
            FROM timetables t
