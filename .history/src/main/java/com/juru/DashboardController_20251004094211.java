...
public class DashboardController {
    ...
    // Example: Calculate today's attendance percentage. This method is defensive and
    // returns 0 if the attendance table or required data is not available.
    private int getTodaysAttendance(Connection conn) {
        ...
        try {
            ...
            // Likely the attendance table doesn't exist in the user's DB; log and
            // return 0 so the UI can load. Caller will display '0%'.
            e.printStackTrace();
            return 0;
        }
    ...
    private void manageAttendance(MouseEvent event) {
        SceneLoader.loadScene("AttendanceView.fxml", "Attendance Management");
    }

    @FXML
    private void openReports(MouseEvent event) {
        SceneLoader.loadScene("ReportsView.fxml", "Reports & Analytics");
    }
}
