...
public class TeacherDashboardController {
    ...
    private void openViewAttendance() {
        try {
            // The resource previously referenced (/com/juru/view-attendance.fxml) does not exist.
    ...
    private void openTimetable() throws Exception {
        try {
            rootPane.setCenter(FXMLLoader.load(getClass().getResource("/com/juru/TeacherTimetabl.fxml")));
        } catch (Exception ex) {
            ex.printStackTrace();
            Alert alert = new Alert(Alert.AlertType.ERROR, "Failed to load View Marks view: " + ex.getMessage());
            alert.showAndWait();
