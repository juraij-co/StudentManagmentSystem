...
public class StudentAttendanceController {
    ...
    private void loadMonthlyAttendance() {
        ...
        // Load timetable with subject names
        Map<String, Map<Integer, SubjectInfo>> timetableMap = new HashMap<>();
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement pst = conn.prepareStatement(
                     "SELECT t.day_of_week, t.period_no, s.id AS subject_id, s.name AS subject_name " +
                     "FROM timetables t " +
            ...
            while (rs.next()) {
                ...
                timetableMap.computeIfAbsent(day, k -> new HashMap<>())
        ...
        for (int day = 1; day <= daysInMonth; day++) {
            ...
            for (int period = 1; period <= periods; period++) {
                SubjectInfo subjInfo = timetableMap.getOrDefault(dow.name(), new HashMap<>()).get(period);
